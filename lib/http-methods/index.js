...
const handleError = err => {
  if (err.response && err.response.body) {
    ...
    const error = {
      ...
      body: `${body?.code} - ${body?.error_messages?.base?.[0]}`,
    };
    return error;
  } else {
...
const setAuthHeaders = (apiKey, origin, version) => {
  let headers = {
...
const setHttpMethods = (LatitudeSh, apiKey, origin, version) => {
  LatitudeSh._headers = setAuthHeaders(apiKey, origin, version);
