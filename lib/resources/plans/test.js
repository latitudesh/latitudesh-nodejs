...
describe('get managed databases plans', () => {
  it('call get request with right params', async () => {
    const path = '/plans/managed_databases';
    LatitudeSh._get = jest.fn(() => {
      return {
        body: {
          success: true,
    ...
    LatitudeShApi.Plans.managedDatabases(searchParams);
    await expect(LatitudeSh._get).toHaveBeenCalledWith(
      path,
      LatitudeSh._headers,
      searchParamsParsed
  ...
  it('call get request with wrong params', async () => {
    ...
    const error = new Error('Async error');
    LatitudeSh._get = jest.fn().mockRejectedValue(error);
    await LatitudeShApi.Plans.managedDatabases().catch(e => {
      expect(e).toBe(error);
    });
  });
});
