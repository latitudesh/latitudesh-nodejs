...
describe("manage power", () => {
  it("call put request with right params", async () => {
    ...
    maxihostApi.Device.Actions.managePower(deviceId, searchParams);
    await expect(Maxihost._put).toHaveBeenCalledWith(path, searchParamsParsed, Maxihost._headers);
  });

  it("call get request with wrong params", async () => {
    const error = new Error("Async error")
    Maxihost._post = jest.fn().mockRejectedValue(error);
    await maxihostApi.Device.Actions.reinstall(deviceId, data).catch(
...
describe("get remote access", () => {
  it("call get request with right params", async () => {
    const path = "/devices/" + deviceId + "/remote_access";
    Maxihost._get = jest.fn(() => {
      return { "body": { "success": true } }
    });
    await maxihostApi.Device.Actions.getRemoteAccess(deviceId);
  ...
  it("call get request with wrong params", async () => {
    ...
    await maxihostApi.Device.Actions.getRemoteAccess(deviceId).catch(
      e => { expect(e).toBe(error) }
    );
  })
})
...
describe("get reinstall", () => {
  it("call get request with right params", async () => {
    const path = "/devices/" + deviceId + "/reinstall";
    Maxihost._get = jest.fn(() => {
      return { "body": { "success": true } }
    });
    await maxihostApi.Device.Actions.getReinstall(deviceId);
    expect(Maxihost._get).toHaveBeenCalledWith(path, Maxihost._headers);
  });
  ...
  it("call get request with wrong params", async () => {
    const error = new Error("Async error")
    Maxihost._get = jest.fn().mockRejectedValue(error);
    await maxihostApi.Device.Actions.getReinstall(deviceId).catch(
      e => { expect(e).toBe(error) }
    );
  })
})
...
describe("schedule reinstall", () => {
  it("call post request with right params", async () => {
    ...
    maxihostApi.Device.Actions.reinstall(deviceId, data);
